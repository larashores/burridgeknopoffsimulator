...
class RungeKuttaMethod(OdeSolver, ABC):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._total_weight = sum(self.WEIGHT_COEFFICIENTS)

    def _step(self):
        derivative = 0
        average_derivative = 0
        for i in range(len(self.WEIGHT_COEFFICIENTS)):
            derivative =  self._difeqs(self._time + self._step_size * self.STEP_WEIGHTS[i],
                                       self._current_values + derivative*self.STEP_WEIGHTS[i]) * self._step_size
            average_derivative += derivative * self.WEIGHT_COEFFICIENTS[i]
        self._current_values += average_derivative / self._total_weight
